...
static int stm32_sdmmc_access_init(struct disk_info *disk)
{
	...
	/* If PM device runtime is not enabled, ensure the hardware
	 * is activated once at init */
	if (!IS_ENABLED(CONFIG_PM_DEVICE_RUNTIME)) {
...
static int stm32_sdmmc_access_deinit(const struct device *dev)
{
	...
	/* If PM device runtime is not enabled, manually deactivate hardware
	 * once on deinit */
	if (!IS_ENABLED(CONFIG_PM_DEVICE_RUNTIME)) {
		int err = stm32_sdmmc_hw_deactivate(dev);
